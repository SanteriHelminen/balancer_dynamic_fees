...
with reserve_changes as (
    ...
    from {{ ref('fct_mainnet_poolbalancechanged') }}  as pool_balance_changed
    inner join {{ ref('dim_mainnet_decimals') }} as token_metadata
        on
    ...
    from {{ ref('fct_mainnet_poolbalancemanaged') }}  as pool_balance_managed
    inner join {{ ref('dim_mainnet_decimals') }} as token_metadata
        on
    ...
    from {{ ref('fct_mainnet_swap') }} as swaps
    inner join {{ ref('dim_mainnet_decimals') }} as token_metadata
        on
