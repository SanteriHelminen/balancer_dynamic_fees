...
with final as (
    select
        ...
        divide(abs(trades.amount_out), pow(10, cast(tokens1.decimals as numeric)))
            as token_out,
    from {{ ref('fct_arbitrum_swap') }} as trades
    left join {{ ref('dim_arbitrum_decimals') }} as tokens0
        on
            trades.token_in = tokens0.token_address
    left join {{ ref('dim_arbitrum_decimals') }} as tokens1
        on
            trades.token_out = tokens1.token_address
    left join {{ ref('fct_arbitrum_fee_change') }} as fee_tier_updates
        on
