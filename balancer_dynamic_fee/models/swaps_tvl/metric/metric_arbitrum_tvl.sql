...
with reserve_changes as (
    ...
    from {{ ref('fct_arbitrum_poolbalancechanged') }}  as pool_balance_changed
    inner join {{ ref('dim_arbitrum_decimals') }} as token_metadata
        on
    ...
    from {{ ref('fct_arbitrum_poolbalancemanaged') }}  as pool_balance_managed
    inner join {{ ref('dim_arbitrum_decimals') }} as token_metadata
        on
    ...
    from {{ ref('fct_arbitrum_swap') }} as swaps
    inner join {{ ref('dim_arbitrum_decimals') }} as token_metadata
        on
